...
#
#=============================================================================
# Copyright (C) 2011 Simone Rossetto <simro...@gmail.com>
# 
...
if(EXISTS ${_exif_pkg_config_file})
    ...
    string(REGEX REPLACE "^.*Version: ([0-9]+).*$" "\\1" EXIF_VERSION_MAJOR 
"${_exif_version}")
    string(REGEX REPLACE "^.*Version: [0-9]+\\.([0-9]+).*$" "\\1" 
EXIF_VERSION_MINOR  "${_exif_version}")
    string(REGEX REPLACE "^.*Version: [0-9]+\\.[0-9]+\\.([0-9]+).*$" "\\1" 
EXIF_VERSION_PATCH "${_exif_version}")
    set(EXIF_VERSION_STRING 
"${EXIF_VERSION_MAJOR}.${EXIF_VERSION_MINOR}.${EXIF_VERSION_PATCH}")
...
if(EXIF_USE_STATIC_LIB)
    ...
    if(EXISTS ${_exif_static_lib})
        ...
        message(WARNING "EXIF static library does not exist, using the shared 
one.")
